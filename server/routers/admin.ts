...
export const adminRouter = router({
  ...
  changeEmail: adminProcedure
    ...
    .mutation(async (opts) => {
      ...
      const isPasswordCorrect = await bcrypt.compare(password, user.confirmationPassword!);
  ...
  createConfirmationPassword: adminProcedure
    ...
    .mutation(async (opts) => {
      ...
      const confirmationPasswordHash = await bcrypt.hash(confirmationPassword, salt);
  ...
  changeConfirmationPassword: adminProcedure
    ...
    .mutation(async (opts) => {
      const { actualConfirmationPassword, newConfirmationPassword } = opts.input;
      const { id } = opts.ctx.admin;
      ...
      const isPasswordCorrect = await bcrypt.compare(actualConfirmationPassword, admin.confirmationPassword!);
      ...
      const salt = await bcrypt.genSalt(10);
      const confirmationPasswordHash = await bcrypt.hash(newConfirmationPassword, salt);
