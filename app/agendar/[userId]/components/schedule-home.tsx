...
export function ScheduleHome({ userId, hasImage, imageSrc, userName, isLoading }: ScheduleHomeProps) {
  if (isLoading) {
    return (
      <div className="w-full max-w-2xl bg-white rounded-3xl p-6 flex flex-col gap-6 items-center">
        ...
        <span className="text-base font-medium text-foreground/70 text-center">
          Encontre o horário perfeito para você com apenas alguns cliques. Escolha o serviço desejado, selecione o dia e
          horário disponível, e finalize seu agendamento. Nosso sistema é rápido, simples e feito para garantir sua
          conveniência.
        </span>
  ...
  return (
    <div className="w-full max-w-2xl bg-white rounded-3xl p-6 flex flex-col gap-6 items-center">
      <div className="flex flex-col gap-2 items-center">
        <h2 className="text-2xl font-semibold text-center text-skin-primary">Agende seu atendimento com facilidade!</h2>

        <div className="relative size-24 rounded-full overflow-hidden bg-skin-primary flex items-center justify-center">
          {hasImage ? (
            <Image src={imageSrc!} alt="Foto de perfil" fill className="object-cover object-center" />
          ) : (
            <UserRound color="#FFF" className="size-12" />
          )}
        </div>

        <span className="text-xl text-skin-primary font-semibold">{userName}</span>
      </div>
