...
function CancelPlanConfirmation({ subscriptionId }: CancelPlanConfirmationProps) {
  const [isOpen, setOpen] = useState<boolean>(false);

  const { mutate: cancelPlan, isPending } = trpc.userRouter.cancelPlan.useMutation({
    onSuccess: (res) => {
    ...
    <AlertDialog open={isOpen}>
      ...
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            disabled={isPending}
            onClick={() => cancelPlan({ subscriptionId })}
...
function PlanComponent() {
  ...
  useEffect(() => {
    if (data !== undefined) {
      if (data.error && !data.plan && data.message === "Plano inativo") {
        console.log("Plano inativo");
        return;
      }
              ...
              <div className="w-full grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                <div className="w-full p-4 bg-skin-primary rounded-2xl flex flex-col justify-between gap-4">
                  ...
                  <div className="w-full flex flex-col items-center gap-4">
                    ...
                    <form className="w-full" action="/api/reactive-checkout-session" method="POST">
